-- =============================================================================
-- Diagram Name: v0_5
-- Created on: 3/14/2013 9:30:54 PM
-- Diagram Version: 436
-- =============================================================================
...
CREATE TABLE `pv_group` (
  ...
  `pvg_version` varchar(50),
  `pvg_version_desc` varchar(255),
  `pvg_status` enum('in progress','complete','invalid'),
...
CREATE TABLE `workflow_type` (
  ...
  `workflow_type_description` varchar(50),
...
CREATE TABLE `person__role` (
  ...
  `person_id` int(11),
  `max_clearance_id` int(11),
  PRIMARY KEY(`person__role_id`),
  CONSTRAINT `Ref_157` FOREIGN KEY (`max_clearance_id`)
...
CREATE TABLE `cmpnttype__cognizant` (
  ...
  `person_id` int(11),
  `role_id` int(11),
  `cmpnt_type_status` enum('in progress','complete'),
  `cmpnt_type_status_date` datetime,
  ...
  CONSTRAINT `Ref_191` FOREIGN KEY (`person_id`)
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `Ref_243` FOREIGN KEY (`role_id`)
    REFERENCES `role`(`role_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION
)
...
-- Drop table lattice__latticedocument
DROP TABLE IF EXISTS `lattice__latticedocument`;

CREATE TABLE `lattice__latticedocument` (
  `lattice__latticedocument_id` int(11) NOT NULL AUTO_INCREMENT,
  `lattice_id` int(11),
  `lattice_document_id` int(11),
  PRIMARY KEY(`lattice__latticedocument_id`),
  CONSTRAINT `Ref_01` FOREIGN KEY (`lattice_document_id`)
    REFERENCES `lattice_document`(`lattice_document_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
